...
function ViewSiteLink({ onNavigate }: { onNavigate?: () => void }) {
  return (
    <Link
      href="/"
      onClick={onNavigate}
      className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-fg-muted hover:bg-surface-2 hover:text-fg"
    >
...
export default function Shell({
  ...
  return (
    <div className="flex min-h-screen">
      {/* Desktop sidebar */}
      <aside className="hidden w-60 shrink-0 border-r border-line bg-surface-1 px-4 py-6 lg:block">
        <Link href="/dashboard" className="block px-2" aria-label="First Chair dashboard">
          <Wordmark markClassName="h-7 w-7 text-fg" textClassName="text-lg text-fg" />
        </Link>
      ...
      <div className="min-w-0 flex-1">
        {/* Mobile top bar */}
        <div className="flex items-center justify-between border-b border-line bg-surface-1 px-4 py-3 lg:hidden">
          <Link href="/dashboard" aria-label="First Chair dashboard">
            <Wordmark markClassName="h-6 w-6 text-fg" textClassName="text-base text-fg" />
          </Link>
          <button
            type="button"
            aria-label="Open menu"
            onClick={() => setMenuOpen(true)}
            className="cursor-pointer rounded-lg p-1 text-fg-muted hover:bg-surface-2"
          >
        ...
        {menuOpen && (
          <div className="fixed inset-0 z-50 lg:hidden">
            <div
              ...
              aria-hidden
            />
            <nav className="absolute right-0 top-0 flex h-full w-64 flex-col gap-1 bg-surface-1 px-4 py-6 shadow-card-hover">
        ...
        <header className="hidden items-center justify-between gap-4 border-b border-line bg-surface-1 px-6 py-3 lg:flex">
          {brands.length > 1 ? (
            <label className="flex items-baseline gap-2 text-sm">
              <span className="notation text-fg-muted">Firm</span>
              <select
                value={currentBrandId ?? brands[0]?.id}
                onChange={(e) => router.push(`/dashboard?brand=${e.target.value}`)}
                className="cursor-pointer border-0 border-b border-line-strong bg-transparent py-1 pr-6 font-semibold text-fg focus:border-brand-500 focus:outline-none"
              >
                {brands.map((b) => (
            ...
            <span className="text-sm">
              <span className="notation text-fg-muted">Firm</span>{" "}
              <span className="font-semibold text-fg">{brandName ?? "—"}</span>
            </span>
          )}
