...
const FROM = process.env.EMAIL_FROM ?? "Rankwell <onboarding@resend.dev>";
...
<div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:560px;margin:0 auto;color:#1a2b3c">
  <h1 style="font-size:20px;color:#153E75;margin:0 0 4px">${esc(title)}</h1>
  ${inner}
  <p style="font-size:12px;color:#64748b;margin-top:24px;border-top:1px solid #e2e8f0;padding-top:12px">
    Rankwell — AI visibility for law firms. Scores from ChatGPT, Gemini and Perplexity.
  </p>
</div>`;
...
export function renderAuditEmail(a: AuditEmailInput): { subject: string; html: string } {
  const rows = a.engines
    ...
    <tr>
      ...
      <td style="padding:8px 0;text-align:right;color:#64748b">${e.mentionedInPrompts}/${e.totalPrompts} prompts${
  ...
  const inner = `
    ...
    <p style="font-size:32px;font-weight:700;color:#153E75;margin:8px 0">${a.overallScore}<span style="font-size:16px;color:#64748b">/100 visibility</span></p>
    <table style="width:100%;border-collapse:collapse;font-size:14px">${rows}</table>
    <a href="${APP_URL}/login" style="display:inline-block;margin-top:16px;background:#153E75;color:#fff;text-decoration:none;padding:10px 18px;border-radius:8px;font-weight:600">Track this daily — start free trial</a>`;
...
export function renderWeeklyEmail(w: WeeklyEmailInput): { subject: string; html: string } {
  ...
  const color = w.delta > 0 ? "#059669" : w.delta < 0 ? "#dc2626" : "#64748b";
  const inner = `
    <p style="font-size:14px">This week for <strong>${esc(w.firmName)}</strong>:</p>
    <p style="font-size:32px;font-weight:700;color:#153E75;margin:8px 0">${w.score}<span style="font-size:16px;color:#64748b">/100</span>
      <span style="font-size:16px;color:${color};margin-left:8px">${arrow} ${w.delta >= 0 ? "+" : ""}${w.delta}</span></p>
    <p style="font-size:14px;color:#64748b">Mentioned in ${Math.round(w.mentionRate * 100)}% of tracked prompts.</p>
    ${w.topRecommendation ? `<p style="font-size:14px"><strong>Fix first:</strong> ${esc(w.topRecommendation)}</p>` : ""}
    <a href="${APP_URL}/dashboard" style="display:inline-block;margin-top:16px;background:#153E75;color:#fff;text-decoration:none;padding:10px 18px;border-radius:8px;font-weight:600">View dashboard</a>`;
  return {
